        ...
        for mb in micro_bsz:
            for i, sp_mode in enumerate(sp):
                for ckpt in checkpoint:
                    ...
                    print(log_name)
                    command = f"srun -p llm_t -x HOST-10-140-66-5,HOST-10-140-60-179 -N 8 -n 64 --ntasks-per-node=8 --gpus-per-task=1 --time=20 python train.py --config {write_file} --profiling 2>&1 | tee ./fstp_logs/{log_name}.log"
