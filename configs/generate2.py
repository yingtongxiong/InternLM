...
sp = ["intern", "intern"]
intern_overlap = [True, True]
memory_pool = [True, False]
checkpoint = [True]
        ...
        for seq in seq_length:
            for i, sp_mode in enumerate(sp):
                for ckpt in checkpoint:
                    ...
                    line = line.replace("{memory_pool}", str(memory_pool[i]))
                    line = line.replace("{checkpoint}", str(ckpt))
                    output_file_name = str(seq) + "_" + str(sp_mode) + "_overlap_" + str(intern_overlap[i]) + "_mempool_" + str(memory_pool[i]) + "_ckpt_" + str(ckpt) + ".py"
                    ...
                    print(log_name)
                    command = f"srun -p llm_s -N 1 -n 8 --ntasks-per-node=8 --gpus-per-task=1 python train.py --config {write_file} --profiling 2>&1 | tee ./fstp_logs_mem_pool/{log_name}.log"
